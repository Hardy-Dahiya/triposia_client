...
// getFlightsDataList
const getFlightsDataList = async (dep_iata: string | number | null) => {
  try {
    const URL = `${url}/flight/from_to_list?iata_code_dep=${dep_iata}`;
    return await axios.get(URL, {});
  } catch (error) {
    if (axios.isAxiosError(error)) {
      if (error.response && error.response.status === 401) {
...
// getDistance
const getDistance = async (dep_iata: string | number | null, arr_iata: string | number | null) => {
  try {
    const URL = `${url}/flight/distance?dep_iata=${dep_iata}&arr_iata=${arr_iata}`;
    return await axios.get(URL, {});
  } catch (error) {
    if (axios.isAxiosError(error)) {
      if (error.response && error.response.status === 401) {
...
export { getFlightsData, getFlightsToData, getFlightsRouteData, getDistance, getFlightsDataList };
